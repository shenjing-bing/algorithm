...
public class _26_删除有序数组中的重复项 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(removeDuplicates(arr1,2,arr1[arr1.length-1]));
        System.out.println(Arrays.toString(arr1));

    }
    /**
     * 双指针方法,因为数组相对有序,那么重复的元素一定会相邻,定义前后两个指针,p,q，每次比较两个指针上的值，
     * 如果相等则q++;
     * 如果不相等，则将当前q位置上的值与p+1上的值做交换，然后p++，q++;
     * @param nums 源数组
     * @return 新数组长度
     */
    ...
    /**
     * 通用解法 ,「最多保留 1 位」修改为「最多保留 k 位」。
     * 对于前k个数字可以直接保留进入目标数组，
     * 对于后面的任意数字，能不能进入目标数组的前提是，如果当前数字与原数组的前k位的数字不想等，则进入目标数组
     * @param nums 源数组
     * @return 新数组长度
     */
